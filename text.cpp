...
float text_t::fontITOF = (float) pow (255.0f, -1.0f);
...
void text_t::DrawChar(char c, int x, int y, int size, int shadow)
{
   if (!gradient && !shadow)
      glColor4fv(fgColor);
...
void text_t::RenderChar (char c, int x, int y, int size)
{
   if (shadow)
   {
      if (!bold)
...
int text_t::SlashParser(char *buffPtr, int *x, int *y)
{
   int ret = 0;
...
#define vCopy(d,x,y) {d[0] = x; d[1] = y;}
   ...
   tIncX = (float) pow ((float)blockCol, -1);
   tIncY = (float) pow ((float)blockRow, -1);
