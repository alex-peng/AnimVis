...
void main(void)
{
	...
	output_vec = vec4(p1.x, p1.y, p1.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p1.x, p2.y, p1.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p2.x, p2.y, p1.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p2.x, p1.y, p1.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4(p1.x, p1.y, p1.z, 1.0 );
	...
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p1.x, p1.y, p2.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p1.x, p2.y, p2.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p2.x, p2.y, p2.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p2.x, p1.y, p2.z, 1.0 );
	gl_Position = gl_ModelViewProjectionMatrix * output_vec;
	gl_FrontColor = color;
	EmitVertex();
	output_vec = vec4( p1.x, p1.y, p2.z, 1.0 );
